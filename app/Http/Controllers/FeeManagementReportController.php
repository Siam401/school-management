...
class FeeManagementReportController extends Controller
{
    ...
    {
        $feeDetails = StudentCollection::select(
            'student_collections.class_id',
            'classes.class_name',
            DB::raw('SUM(CASE WHEN fee_heads.name = "Semester Exam" THEN student_collection_details.fee_and_fine_paid ELSE 0 END) as semester_exam'),
            DB::raw('SUM(CASE WHEN fee_heads.name = "Tuition Fees" THEN student_collection_details.fee_and_fine_paid ELSE 0 END) as tuition_fees'),
            DB::raw('SUM(CASE WHEN fee_heads.name = "Transport Fees" THEN student_collection_details.fee_and_fine_paid ELSE 0 END) as transport_fees'),
            DB::raw('SUM(CASE WHEN fee_heads.name = "ICT Fees" THEN student_collection_details.fee_and_fine_paid ELSE 0 END) as ict_fees'),
            DB::raw('SUM(student_collection_details.fee_and_fine_paid) as total_amount')
        )
            ->join('student_collection_details', 'student_collections.id', '=', 'student_collection_details.student_collection_id')
            ->join('fee_heads', 'fee_heads.id', '=', 'student_collection_details.fee_head_id')
            ->join('classes', 'student_collections.class_id', '=', 'classes.id')
            ->where('student_collections.class_id', $request->class_id)
            // ->whereDate('student_collections.invoice_date', $date) 
            ->groupBy('student_collections.class_id')
            ->get();

        if ($feeDetails && count($feeDetails) > 0) {
            return view('backend.fee_collection_report.partials.fee-head-class-collection', compact('feeDetails'));
        }
    ...
    public function getPaymentInfoDetails($id)
    {
        ...
        $studentCollections = StudentCollection::select(
            ...
            ->join('student_collection_details_sub_heads', 'student_collection_details_sub_heads.student_collection_id', '=', 'student_collections.id')
            ->join('student_collection_details', 'student_collections.id', '=', 'student_collection_details.student_collection_id')
